...
void BST::BranchWalk(Node *root, Node **&result, int &index) {
  if (root != nullptr) {
    BranchWalk(root->left, result, index);
    BranchWalk(root->right, result, index);
...
int main() {
  ...
  int index = 0;
  bst.BranchWalk(bst.GetRoot(), result, index);
