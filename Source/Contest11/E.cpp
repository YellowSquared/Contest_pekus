...
class Treap {
 public:
  ...
  void InsertCount(int key, std::mt19937 &gen);
  void Remove(int key);
  Node *Min(Node *node);
  Node *Max(Node *node);
  void ExclusiveMerge(Node *node, std::mt19937 &gen);
  Node *GetRoot() {
    return root_;
  }
...
void Treap::ExclusiveMerge(Node *node, std::mt19937 &gen) {
  if (node != nullptr) {
    ExclusiveMerge(node->left, gen);
    InsertCount(node->key, gen);
    ExclusiveMerge(node->right, gen);
  }
}
...
void Treap::InsertCount(int key, std::mt19937 &gen) {
...
int main() {
  ...
  for (int i = 0; i < n2; i++) {
    int val = 0;
    std::cin >> val;
    treap2.Insert(val, gen);
  }

  treap1.ExclusiveMerge(treap2.GetRoot(), gen);
  int count = 0;
  treap1.PrintCount(count);
  std::cout << count;
  std::cout << '\n';
  treap1.Print();
}
