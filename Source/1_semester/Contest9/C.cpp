...
class BST {
 ...
 public:
  ...
  void PrintPreLeaves() {
    if (root_ != nullptr) {
      PrintPreLeavesFromSmth(root_);
    }
  }

  void PrintPreLeavesFromSmth(Node* smth_node) {
    if ((smth_node->left == nullptr) != (smth_node->right == nullptr)) {
      if (smth_node->left == nullptr) {
        std::cout << smth_node->value << ' ';
        PrintPreLeavesFromSmth(smth_node->right);
      } else {
        PrintPreLeavesFromSmth(smth_node->left);
        std::cout << smth_node->value << ' ';
      }
    } else if (smth_node->left != nullptr) {
      PrintPreLeavesFromSmth(smth_node->left);
      PrintPreLeavesFromSmth(smth_node->right);
    }
  }
