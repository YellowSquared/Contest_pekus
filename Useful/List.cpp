...
NodeList(0, 0xeafeaf) -> NodeList(1, 0xaefaef) -> NodeList(2, 0x0)
        /\     /\
        ||     ||
  значение    указатель на следующий элемент    
*/
...
|| || ||   Меняем укаказатель в предпоследнем элементе на nullptr, 
|| || ||   удаляем последний узел из памяти и меняем указатель хвоста в списке.
\/ \/ \/
